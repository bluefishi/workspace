...
public class MFCC
{
 ...
 private Matrix getMelFilterBanks()
  ...
  /*apple overwrite this method the adjust the work*/
  @SuppressWarnings("unchecked")
	public Vector process(String filename,MFCC mfcc)
	  {
		 ...
		 AudioPreProcessor in = new AudioPreProcessor(audioInputStream, 8000);
		 try {
			result = mfcc.process(in);
  ...
  /*apple add this method the adjust the work*/
  @SuppressWarnings("unchecked")
	public void saveMFCC(Vector result,String filename,String outpath) throws IOException{
		  File file = new File(outpath+"\\"+filename);
		  BufferedWriter bw = new BufferedWriter(new FileWriter(file));
		  for(int i=0;i<result.size();i++)
		  {
			  double[] temp = (double[]) result.get(i);
